...
# %%
# 重命名列
df.columns = ['Temperature', 'Frequency', 'Core Loss', 'Waveform', 'material']  # 修正错别字
print(df.columns)
...
# 材料对磁芯损耗影响
material_loss.plot(kind='bar', ax=ax[1, 0], title='Effect of Material on Core Loss')
ax[1, 0].set_ylabel('Core Loss (W/m^3)')

# 温度和波形协同作用
sns.heatmap(temp_waveform_loss, cmap='coolwarm', annot=True, ax=ax[1, 1])
ax[1, 1].set_title('Interaction Between Temperature and Waveform')

# 温度和材料协同作用
sns.heatmap(temp_material_loss, cmap='coolwarm', annot=True, ax=ax[2, 0])
ax[2, 0].set_title('Interaction Between Temperature and Material')

# 波形和材料协同作用
sns.heatmap(waveform_material_loss, cmap='coolwarm', annot=True, ax=ax[2, 1])
ax[2, 1].set_title('Interaction Between Waveform and Material')
...
# 输出结果
feature_names = ['Temperature', 'Frequency'] + list(encoder.get_feature_names_out(['Waveform', 'material']))
...
# 绘制回归系数条形图
coef_df.set_index('Feature').plot(kind='bar', figsize=(10, 6), title='Regression Coefficients Impact on Core Loss')
